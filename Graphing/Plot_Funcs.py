...
class Plot_Funcs():
    ...
    def Make_Plots(self):
        ...
        Robert:
            ...
            so this function simply parses through the user inout.
    ...
    def Collect_CNA(self, Sig):
        try:
            ...
            return [ self.Meta['cna'][x][Index] for x in range(len(self.Meta['cna'])) ]
        except KeyError:
            with open(self.Base+'Plotting_Info.txt', "a") as f:
    ...
    def Collect_CNA_error(self, Sig):
        try:
            ...
            return [ self.Err['cna'][x][Index] for x in range(len(self.Err['cna'])) ]
        except KeyError:
            with open(self.Base+'Plotting_Info.txt', "a") as f:
                f.write("\nData not found in metadata\n")
            return None
    ...
    def agcn_heat(self, Name = 'agcn_Heat.png'):
        ...
        try:
            ...
            Ticks = [ "{:3f}".format(b) for b in Bins ]
            
            Heat = ( np.asanyarray(Heat) ).transpose()
            
            plt.figure(figsize  = (18,9 ) )
            ax = sns.heatmap(Heat, cmap = 'hot', yticklabels = Ticks)
    ...
    def plot_stats(self, Stats = [], Species = None, Quants = [], Temp = False, Errors = False):
        if self.Errors is True:
            Errors =  True
    ...
    def cna_plot(self, Name = 'CNA_Time', Frames = [], Errors = False):
        ...
        for Frame in Frames:
            try:
                ...
                ax = plt.bar( X_CNA, self.Meta['cna'][Frame], yerr = self.Err['cna'][Frame], tick_label = X_CNA )
                plt.xlabel("CNA Signature", fontsize = 14)
                plt.ylabel("Probability", fontsize = 14)
                plt.xticks(rotation=90,fontsize = 14)
                try:
                    plt.text( X_CNA[-7], 0.8*np.amax(self.Meta['cna'][Frame]), 
    ...
    def com_full_plot(self, Frames = [], Errors =  False):
        ...
        for Frame in Frames:
            fig, ax = plt.subplots()
            fig.set_size_inches(9,3)
            ax.plot(self.Meta['CoMSpace'], self.Meta['CoMDist'], color='k')
            if Errors is True:
                ax.fill_between(self.Meta['CoMSpace'] ,
                                self.Meta['CoMDist'] + self.Err['CoMDist'],
                                self.Meta['CoMDist'] - self.Err['CoMDist'],
                                color='k', alpha = 0.25)
            ax.set_xlabel('Distance (Angstrom)')
            ax.set_ylabel('RDF')
            ax.text(a[5], 0.65*max(b), "Full System\nTime: %sps\nTemp: %sK" %(self.Meta['SimTime'][Frame], "{:.1f}".format(self.Meta['Temp'][Frame])))
    ...
    def cna_traj(self, Sigs = [], Errors = False):
        ...
        for x in Sigs:
            try:
                ax.plot(Time, self.Collect_CNA(x), label = '(%s %s %s)' %(x[0],x[1],x[2]))
                if Errors is True:
    ...
    def h_c(self, Errors = False):
        ...
        ax.plot(Time[1:], self.Meta['h'], label = 'Collectivity')
        ax.plot(Time[2:-1], self.Meta['c'], label = 'Concertedness')
        if Errors is True:
